...
// component form
import Tambah from './tempatSambung/kelompok/Tambah';
import Detail from './tempatSambung/kelompok/Detail';
import Edit from './tempatSambung/kelompok/Edit';
...
export default function DataDesa() {
  ...
  const fetchData = async () => {
    try {
      loading.value = true;
      const response = await fetchClient().get(`/api/kelompok/list?page=${page.value}&per-page=${rows.value}&keyword=${globalFilter.value}`);
      data.value = response.data.data_kelompok.data;
      totalRecords.value = response.data.data_kelompok.total;
      loading.value = false;
    } catch (err) {
      error.value = err;
      loading.value = false;
  ...
  const DetailDataFetch = async () => {
    try {
      const response = await fetchClient().post(`/api/kelompok/edit`, { id: dataUserId.value });
      const userDataArray = response.data.data_kelompok;
  ...
  const onDeleteData = async () => {
    try {
      const response = await fetchClient().delete(`/api/kelompok/delete/`, { data: { id: selectedData.value.id } });
    ...
    <>
      <div className="card card-main-content">
        <div className="card-body">
          <div className="fw-semibold fs-3 mb-4">Tempat Kelompok</div>
          <Toast ref={toastRef} />
          ...
          <div className="table-responsive">
            <DataTable value={data.value} showGridlines stripedRows loading={loading.value} dataKey="id" globalFilterFields={['user_name', 'email', 'status']} emptyMessage="Belum ada data" className="p-datatable-sm">
              ...
              />
              <Column field="nama_kelompok" header="Nama" />
