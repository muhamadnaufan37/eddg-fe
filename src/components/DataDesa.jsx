...
// component form
import Tambah from './tempatSambung/desa/Tambah';
import Detail from './tempatSambung/desa/Detail';
import Edit from './tempatSambung/desa/Edit';
...
export default function DataDesa() {
  ...
  const fetchData = async () => {
    try {
      loading.value = true;
      const response = await fetchClient().get(`/api/desa/list?page=${page.value}&per-page=${rows.value}&keyword=${globalFilter.value}`);
      data.value = response.data.data_desa.data;
      totalRecords.value = response.data.data_desa.total;
      loading.value = false;
    } catch (err) {
      error.value = err;
      loading.value = false;
  ...
  const DetailDataFetch = async () => {
    try {
      const response = await fetchClient().post(`/api/desa/edit`, { id: dataUserId.value });
      const userDataArray = response.data.data_desa;
  ...
  const onDeleteData = async () => {
    try {
      const response = await fetchClient().delete(`/api/desa/delete/`, { data: { id: selectedData.value.id } });
    ...
    <>
      <div className="card card-main-content">
        <div className="card-body">
          <div className="fw-semibold fs-3 mb-4">Tempat Desa</div>
          <Toast ref={toastRef} />
          ...
          <div className="table-responsive">
            <DataTable value={data.value} showGridlines stripedRows loading={loading.value} dataKey="id" globalFilterFields={['user_name', 'email', 'status']} emptyMessage="Belum ada data" className="p-datatable-sm">
              ...
              />
              <Column field="nama_desa" header="Nama" />
